//====================================================================================================
// Improved Noise reference implementation (Ken Perlin) ÇÃà⁄êAî≈
...
var PerlinNoise = (function () {
    ...
    // private method Ç≈èâä˙âªÇ∑ÇÈÇ∆ this.xxx Ç™ undefined Ç…Ç»ÇÈ
    PerlinNoise.prototype.initialize = function () {
        this.permutation = [
        ...
        this.p = [];
        for (var i = 0; i < 256; ++i)
            this.p.push( this.permutation[ i ] );
        for (var i = 0; i < 256; ++i)
            this.p.push( this.permutation[ i ] );
    }

    PerlinNoise.prototype.noise = function( x, y, z ) {
        var X = (Number)(Math.floor( x ) & 255);
        var Y = (Number)(Math.floor( y ) & 255);
        var Z = (Number)(Math.floor( z ) & 255);
        x -= Math.floor( x );
        y -= Math.floor( y );
        z -= Math.floor( z );
        var u = fade( x );
